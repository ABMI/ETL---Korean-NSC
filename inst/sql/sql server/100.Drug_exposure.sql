...
SELECT convert(bigint, convert(bigint, a.master_seq) + convert(bigint, row_number() over (partition by a.key_seq, a.seq_no order by b.target_concept_id))) as drug_exposure_id,
